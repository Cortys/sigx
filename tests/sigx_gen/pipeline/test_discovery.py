...
@pytest.mark.parametrize(
    ...
    [
        ("T: pkg.types.Bound", "T: pkg.types.Bound"),
        pytest.param(
            "T: pkg.types.Bound = pkg.types.Default",
            "T: pkg.types.Bound = pkg.types.Default",
            marks=pytest.mark.skipif(sys.version_info < (3, 13), reason="type parameter defaults require Python 3.13"),
        ),
    ],
)
def test_discovery_extracts_function_type_params(tmp_path: Path, declaration: str, expected: str) -> None:
    src_root = tmp_path / "src"
    pkg = src_root / "pkg"
    pkg.mkdir(parents=True)
    file_path = pkg / "mod.py"
    file_path.write_text(
        f"def run[{declaration}](x: T) -> T:\n    return x\n",
        encoding="utf-8",
    )
    ...
    assert sig.type_params == (expected,)
